<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="GEN_REPORT2.aspx.cs" Inherits="PROPERTY_RETURNS.FORMS.GEN_REPORT2" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <fieldset style="height: 800px; width: 960px;">
        <legend>General Report (Separated Employees)</legend>
...
<HeaderStyle Width="20px"></HeaderStyle>
        ...
        <HeaderContextMenu CssClass="GridContextMenu GridContextMenu_Default">
            ...
            ConnectionString="<%$ ConnectionStrings:PRConnectionString %>" 
            SelectCommand="SP_GENERAL_REPORT1_SEP" SelectCommandType="StoredProcedure">
                <SelectParameters>
        ...
        ConnectionString="<%$ ConnectionStrings:PRConnectionString %>" 
        SelectCommand="SELECT top 1 '%' as PA,'  All' as PROJECT FROM [ERPHR.NTPCLAKSHYA.CO.IN].[ERPHRDB].[dbo].[EMPMASTERINACTIVE]  union all  SELECT distinct [PA], [PROJECT] FROM [ERPHR.NTPCLAKSHYA.CO.IN].[ERPHRDB].[dbo].[EMPMASTERINACTIVE] where  PA <>'' and PROJECT <> '' order by PROJECT ">
                <SelectParameters>
        ...
        ConnectionString="<%$ ConnectionStrings:PRConnectionString %>" 
        SelectCommand="SELECT top 1 '%' as PSA,'  All' as Location FROM [ERPHR.NTPCLAKSHYA.CO.IN].[ERPHRDB].[dbo].[EMPMASTERINACTIVE] where psa like @authpsa union all SELECT distinct [PSA], [LOCATION] FROM [ERPHR.NTPCLAKSHYA.CO.IN].[ERPHRDB].[dbo].[EMPMASTERINACTIVE] WHERE PA=@PA and psa like @authpsa and PSA <> '' and LOCATION <> '' order by LOCATION">
        <SelectParameters>
        ...
        ConnectionString="<%$ ConnectionStrings:PRConnectionString %>" 
        SelectCommand="SELECT top 1 '%' as PERNR,'  All' as EMPNAME FROM [ERPHR.NTPCLAKSHYA.CO.IN].[ERPHRDB].[dbo].[EMPMASTERINACTIVE] union all SELECT [PERNR], RIGHT([PERNR],6)+' / '+[FIRSTNAME]+' '+[LASTNAME] as EMPNAME FROM [ERPHR.NTPCLAKSHYA.CO.IN].[ERPHRDB].[dbo].[EMPMASTERINACTIVE] WHERE PA=@PA AND PSA=@PSA order by empname">
        <SelectParameters>
        <asp:Parameter Name="PA"  Type="String"/>
